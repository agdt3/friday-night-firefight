...
object Level1Model {
  def initial(viewport: GameViewport): Level1Model = {
    Level1Model(
      ...
      enemies = (EnemyModel(Vertex(170, 120)) :: EnemyModel(Vertex(400, 300)) :: Nil),
  ...
  def update(gameTime: GameTime, inputState: InputState, state: Level1Model): GlobalEvent => Outcome[Level1Model] = {
    case FrameTick => {
      ...
      val updatedPlayer = if isPlayerHit then newPlayer.takeDamage(damage) else newPlayer
  ...
  def updateProjectilePostions(projectiles: List[Projectile], cameraBoundingBox: BoundingBox): List[Projectile] = {
  ...
  def checkEnemyHit(enemy: EnemyModel, projectiles: List[Projectile]): (Boolean, Int) = {
