...
final case class PlayerModel(
  ...
  def getNewLocation(velocity: Vector2): Vertex =
    Vertex(this.location.x + velocity.x, this.location.y + velocity.y)
  ...
  def update(
    ...
    val (player, hasNewProjectiles) = PlayerModel.update(
      this,
      cameraBoundingBox,
      gameMap,
      gameTime,
...
object PlayerModel {
  ...
  def update(
    ...
    val newPosition = updatePlayerPosition(
      player,
      cameraBoundingBox,
      gameMap,
      gameTime,
    ...
    val (lastFired, updatedProjectiles, hasNewProjectiles) =
    ...
    val newStatus = updateStatus(player.status, gameTime)
  ...
  def updatePlayerPosition(
    ...
    val contains = playerControlState.direction match
      ...
      case Vector2(1.0, 0) =>
        cameraBoundingBox.contains(
          newLocation.x + player.hitBox.x / 2,
          newLocation.y
        )
      case Vector2(-1.0, 0) =>
        cameraBoundingBox.contains(
      ...
      case Vector2(0, 1.0) =>
        cameraBoundingBox.contains(
          newLocation.x,
          newLocation.y + player.hitBox.y / 2
        )
      case Vector2(0, -1.0) =>
        cameraBoundingBox.contains(
          newLocation.x,
  ...
  def updatePlayerProjectiles(
    ...
    val shiftedProjectiles =
      newProjectilePositions(player.projectiles, cameraBoundingBox)
    playerControlState.firing match
      case true => {
        val (lastFired, newProjectiles) =
          addProjectiles(player, shiftedProjectiles, gameTime)
        (
          lastFired,
          newProjectiles,
          newProjectiles.length > shiftedProjectiles.length
        )
      }
      case false => (player.lastFired, shiftedProjectiles, false)
  }

  def updateStatus(status: PlayerStatus, gameTime: GameTime): PlayerStatus = {
    if (status.isInvincibleFor == Seconds(0)) {
      return status
    }
    val remainingTime = status.isInvincibleFor - gameTime.delta
  ...
  def addProjectiles(
    ...
    newLastFired < player.rof match
      ...
      case false =>
        (
          Seconds.zero,
          addProjectile(player.location, player.direction) :: projectiles
        )
  }
